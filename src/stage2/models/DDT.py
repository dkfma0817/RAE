...
def _repeat_segments(t, repeat):
    # t: (B, L, D) → (B, L*repeat, D) without real copy
    B, L, D = t.shape
    return t.unsqueeze(2).expand(B, L, repeat, D).reshape(B, L * repeat, D)

def DDTModulate(x, shift, scale):
    B, Lx, D = x.shape
    _, L, _ = shift.shape
    if Lx % L != 0:
        raise ValueError("Token length mismatch in DDTModulate")

    repeat = Lx // L
    if repeat != 1:
        shift = _repeat_segments(shift, repeat)
        scale = _repeat_segments(scale, repeat)
...
def DDTGate(x: torch.Tensor, gate: torch.Tensor) -> torch.Tensor:
    ...
    if repeat != 1:
        ...
        gate = gate.unsqueeze(2).expand(B, L, repeat, D).reshape(B, L * repeat, D)
...
class DiTwDDTHead(nn.Module):
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        if xavier_uniform_init:
            def _basic_init(module):
                # [수정됨] 학습되는 파라미터(requires_grad=True)만 초기화해야 함!
                # SigLIP 같은 Pre-trained 모델을 건드리지 않기 위함
    ...
    def forward(self, x, t, text_embedding, s=None, mask=None):
        
        t = self.t_embedder(t)
        c = nn.functional.silu(t + text_embedding)
