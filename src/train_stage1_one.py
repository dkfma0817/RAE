...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # -------------------------
        # Save recon image every 50 epochs
        # -------------------------
        if (epoch % 200 == 0) and (num_batches > 0):
            rae.eval()
            with torch.no_grad():
                sample_images, _ = next(iter(loader))
                sample_images = sample_images.to(device)

                out = rae(sample_images)
                recon_img = out[0] if (use_vq and isinstance(out, (tuple, list))) else out

                save_path = os.path.join(experiment_dir, f"recon_epoch_{epoch:04d}.png")
                save_image(torch.cat([sample_images, recon_img], dim=0), save_path, nrow=sample_images.size(0))
            rae.train()
