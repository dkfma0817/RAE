...
#         # --- CLS-free decoder variant ---
#         # encoder latent에는 CLS가 없으므로 그냥 patch tokens만 사용
#         x_ = self.interpolate_latent(x)

#         # positional embedding도 patch 부분만 사용
#         hidden_states = x_ + self.decoder_pos_embed[:, 1:, :]
#         # --------------------------------
...
#         # add pos embed (CLS-free)
#         if interpolate_pos_encoding:
#             # (너는 지금 이 옵션 안 쓸 가능성이 커서, 일단 막는 게 안전)
#             raise NotImplementedError("CLS-free variant: interpolate_pos_encoding not supported yet.")
#         else:
#             decoder_pos_embed = self.decoder_pos_embed[:, 1:, :]  # patch pos only

#         hidden_states = x_ + decoder_pos_embed
